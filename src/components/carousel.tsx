...
const Carousel: FunctionComponent<Props> = ({ images }) => {
  useEffect(() => {
    ...
    if (scrollContainer) {
      scrollContainer.onscroll = () => {
        if (
          scrollContainer.scrollLeft + scrollContainer.offsetWidth + 60 >=
  ...
  return (
    <div className="relative w-full">
        ...
        {images.map((image) => (
          <li className="shrink-0 snap-center scroll-smooth p-0">
            <img
              src={image.src}
              alt={image.alt}
              className="max-h-[300px] lg:max-h-[380px]"
            />
          </li>
        ))}
      </ul>
      <button
        id="left-arrow"
        className={navButtonStyle('left-0')}
        onClick={() => scrollImage(-300)}>
        ❮
      </button>
      <button
        id="right-arrow"
        className={navButtonStyle('right-0')}
        onClick={() => scrollImage(300)}>
        ❯
      </button>
    </div>
